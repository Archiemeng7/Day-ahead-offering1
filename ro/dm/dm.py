...
class DataManager(ABC):
    ...
    def generate_dataset(self, n_procs, debug, name_suffix=""):
        ...
        get_path = factory_get_path(self.problem_name)
        out_path = get_path(self.cfg.data_path, self.cfg, "ml_data", suffix=name_suffix)
        print("Saving ML dataset to:", out_path)
        pkl.dump(ml_data, open(out_path, 'wb'), protocol=pkl.HIGHEST_PROTOCOL)
    ...
    def generate_dataset_by_inst(self, n_procs, debug, name_suffix=""):
        """ Generate dataset for training ml models. """
        # seed data collection
        np.random.seed(self.cfg.seed)
